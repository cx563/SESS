...
def pos_entity_sample(sen, context_size, entity_types, entity_masks, entity_start_masks, entity_end_masks,
                      entity_sizes):
    ...
    for e in sen.entities:
        ...
        pos_entity_masks.append(create_entity_mask(*e.span,
                                                   context_size))
        pos_entity_start_masks.append(
            create_entity_s_e_mask(*e.span, context_size, 1))
        pos_entity_end_masks.append(
            create_entity_s_e_mask(*e.span, context_size, 0))
...
def neg_entity_sample(sen, pos_entity_spans, neg_entity_count, max_span_size, token_count, context_size, entity_types,
                      entity_masks, entity_start_masks, entity_end_masks, entity_sizes):
...
def create_entity_sample_mask(entity_masks, entity_types, entity_start_masks, entity_end_masks, entity_sizes,
                              context_size):
    if entity_masks:
...
def train_create_sample(sen, neg_entity_count: int, neg_senti_count: int, max_span_size: int, senti_type_count: int):
    ...
    pos_entity_spans, entity_types, entity_masks, entity_start_masks, entity_end_masks, entity_sizes = pos_entity_sample(
        sen, context_size, entity_types, entity_masks, entity_start_masks, entity_end_masks, entity_sizes)
    neg_entity_spans, entity_types, entity_masks, entity_start_masks, entity_end_masks, entity_sizes = neg_entity_sample(
        sen, pos_entity_spans,
        neg_entity_count, max_span_size, token_count, context_size,
        entity_types, entity_masks, entity_start_masks, entity_end_masks, entity_sizes)
    entity_sample_masks, entity_types, entity_masks, entity_start_masks, entity_end_masks, entity_sizes = create_entity_sample_mask(
        entity_masks, entity_types, entity_start_masks, entity_end_masks, entity_sizes, context_size)
    ...
    assert len(entity_masks) == len(entity_sizes) == len(entity_types) == len(entity_start_masks) == len(
        entity_end_masks)
...
def create_eval_sample(sen, max_span_size: int):
    ...
    if len(pos_tokens) == 0:
        ...
        window_size = 3
        d = {(0, -1): 1}
        for i, position in enumerate(pos_tokens):
            for left in range(max(0, position - window_size), position + 1):
                for right in range(position + 1, min(position + window_size + 2, token_count + 1)):
                    span = sen.tokens[left:right].span
                    if d.get(span) is None:
                        entity_spans.append(span)
                        entity_masks.append(create_entity_mask(*span, context_size))
                        entity_start_masks.append(create_entity_s_e_mask(*span, context_size, 1))
                        entity_end_masks.append(create_entity_s_e_mask(*span, context_size, 0))
                        entity_sizes.append(right - left)
                        d[span] = 1
